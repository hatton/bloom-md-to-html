...
import { ConversionStats, ParsedBook } from './types.js';
...
export class BloomConverter {
  ...
  async convert(inputPath: string, validateOnly: boolean = false): Promise<ConversionStats> {
    ...
    // Initialize parser with input path
    this.parser = new MarkdownParser(resolvedInputPath);

    // Read and parse markdown
    const content = readFileSync(resolvedInputPath, 'utf-8');
    const book = this.parser.parse(content);

    // Show warnings
    const warnings = this.parser.getErrors().filter(e => e.type === 'warning');
  ...
  printStats(stats: ConversionStats): void {
    ...
    console.log(`   Languages: ${stats.languages.join(', ')} (${stats.languages.length} total)`);
    console.log(`   Images: ${stats.images}`);
    console.log('   Layouts:');
    Object.entries(stats.layouts).forEach(([layout, count]) => {
      const layoutName = layout.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
