...
import { BookMetadata, PageContent, ParsedBook, ValidationError } from './types.js';
...
export class MarkdownParser {
  ...
  private parsePage(content: string, metadata: BookMetadata, pageNumber: number): PageContent | null {
    ...
    for (const line of lines) {
      ...
      if (imageMatch) {
        ...
        if (!existsSync(fullImagePath)) {
          this.addWarning(`Image not found: ${imagePath} (page ${pageNumber})`);
        }
        continue;
      }
      ...
      if (langMatch) {
        ...
        if (currentLang && currentText.trim()) {
          page.textBlocks[currentLang] = this.convertMarkdownToHtml(currentText.trim());
        }
        ...
        if (!metadata.languages || !metadata.languages[currentLang]) {
          this.addWarning(`Language '${currentLang}' not found in metadata languages (page ${pageNumber})`);
        }
        continue;
      }
    ...
    // Determine layout
    if (imageFound) {
      page.layout = imagePosition === 'before' ? 'image-top-text-bottom' : 'text-top-image-bottom';
    }
